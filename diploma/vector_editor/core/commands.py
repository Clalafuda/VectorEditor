...
class RemoveLayerCommand(QUndoCommand):
    def __init__(self, document, layer):
        ...
        self.document = document
        self.layer = layer
        self.items_to_reparent = []

    def redo(self):
        self.items_to_reparent = self.layer.childItems()
        self.document._remove_layer(self.layer)
        for item in self.items_to_reparent:
            if self.document.active_layer:
                self.document.active_layer.add_item(item)
            else:
                item.setParentItem(None) # Remove from scene if no active layer
